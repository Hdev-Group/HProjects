...
const UpdateSender = ({ onClose, id, projectid, taskProcesser, taskPriorityy, responders }) => {
    ...
    const handleFormSubmitReal = useCallback(async (e) => {
        ...
        if (document.getElementById('textupdater').value.length > 2000) {
            toast({
                variant: "destructive",
                description: "Message is too long, please shorten it to 2000 characters or less."
            })
        }
        const respondersa = [...responders]; // Create a copy of responders array
        // Append the user id to the responders array if it's not already present
        ...
        try {
            ...
            if (taskPriority !== taskPriorityy || taskProcess !== taskProcesser) {
                ...
                if (taskProcess !== taskProcesser) {
                    await addlog({
                        ...
                        previous: taskProcesser,
                    });
                } else if (taskPriority !== taskPriorityy) {
                    await addlog({
                        projectid: projectid,
                        incidentid: id,
                        userid: userId,
    ...
    }, [id, onClose, addlog, userId, taskPriority, taskProcess, taskTime]);
                            ...
                            <div className="flex flex-col gap-3">
                                <div className='flex w-full gap-3 flex-col'>
                                    ...
                                    <div>
                                        ...
                                        <textarea maxLength={2000} id="textupdater" className='bg-transparent w-full border-neutral-800 text-white border rounded-lg px-3 py-2 resize-y min-h-[8rem]' placeholder={`What is the situation? \nWhat is your next move? \nWhat do you need?`} />
                                    </div>
                                    <div>
